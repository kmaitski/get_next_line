...
/*   Updated: 2017/05/12 14:32:32 by kmaitski         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int intialize_line (char *tmp, char **line, int fd, int read_bytes, int *newline_location);
...
int	intialize_line (char *tmp, char **line, int fd, int read_bytes, int *newline_location)
{
	...
	*newline_location = find_newline(tmp);
	...
	else if (read_bytes)
	{
		...
		if (!read_bytes)
			tmp[len] = '\n';
		*newline_location = intialize_line(tmp, line, fd, read_bytes, newline_location);
	}
	else
...
//		free (line);
	...
	intialize_line(tmp, line, fd, read_bytes, &newline_location);
	newline_location++;
